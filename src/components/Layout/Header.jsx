...
export default function Header({ onMenuOpen }) {
  const { user, currentPage, budgetYears, currentYear, activateBudgetYear, setCurrentYear, expenseRequests } = useApp();
  ...
  return (
    <header className="bg-white border-b border-gray-100 h-14 md:h-16 flex items-center px-4 md:px-6 gap-3 shadow-sm flex-shrink-0">
      ...
      <h1 className="text-base md:text-lg font-bold text-gray-800 flex-1 truncate">
        {PAGE_TITLES[currentPage] || 'לוח מחוונים'}
      </h1>

      {/* Budget Year Selector (hidden on small mobile) */}
      <div className="relative hidden sm:block">
        <button
          onClick={() => setShowYearMenu(p => !p)}
          className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border border-gray-200 text-xs md:text-sm text-gray-600 hover:bg-gray-50 transition-colors"
        >
          <span className="font-medium">{currentYear?.label}</span>
          <ChevronDown size={13} />
        </button>
        {showYearMenu && (
          <>
            ...
            <div className="absolute top-full mt-1 left-0 bg-white rounded-xl shadow-lg border border-gray-100 py-1 z-20 min-w-48">
              {budgetYears.map(yr => (
                <button
                  key={yr.id}
                  onClick={() => { activateBudgetYear(yr.id); setCurrentYear(yr); setShowYearMenu(false); }}
                  className={`w-full text-right px-4 py-2 text-sm hover:bg-gray-50 flex items-center justify-between gap-2 ${yr.id === currentYear?.id ? 'text-teal-600 font-medium' : 'text-gray-700'}`}
                >
                  <span>{yr.label}</span>
      ...
      {/* Notifications */}
      <button className="relative p-1.5 md:p-2 rounded-lg hover:bg-gray-100 transition-colors flex-shrink-0">
