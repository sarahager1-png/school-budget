...
import { Plus, Upload, CheckCircle, Clock, ChevronRight, Trash2, Save, Users, Calendar } from 'lucide-react';
...
const MOCK_EMPLOYEES = [
  { id: 'emp1', name: 'שרה לוי', role: 'מנהלת', monthlySalary: 27000, isActive: true, notes: '' },
  { id: 'emp2', name: 'רחל כהן', role: 'מזכירה', monthlySalary: 8500, isActive: true, notes: '' },
  { id: 'emp3', name: 'דינה פרידמן', role: 'שרת', monthlySalary: 5500, isActive: true, notes: '' },
  { id: 'emp4', name: 'מרים ברק', role: 'יועצת', monthlySalary: 6000, isActive: true, notes: '' },
];
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6" onClick={e => e.stopPropagation()}>
        <h2 className="text-lg font-bold text-gray-800 mb-5">{employee ? 'עריכת עובד' : 'הוספת עובד'}</h2>
        <div className="space-y-4">
          <div>
            <label className="label">שם מלא</label>
            <input className="input" value={form.name} onChange={e => set('name', e.target.value)} placeholder="שם העובד" />
          </div>
          <div>
          ...
          <div>
            ...
            <input className="input" type="number" value={form.monthlySalary} onChange={e => set('monthlySalary', e.target.value)} placeholder="0" />
          </div>
          <div>
        ...
        <div className="flex gap-3 mt-6">
          <button
            onClick={() => { onSave({ ...form, monthlySalary: Number(form.monthlySalary) }); onClose(); }}
...
function PaymentModal({ employee, month, payment, user, onSave, onClose }) {
  ...
  const handlePay = async () => {
    if (isMockMode || !file) {
      onSave({ status: 'paid', notes, receiptUrl: file ? URL.createObjectURL(file) : null });
      onClose();
      return;
    }
    setUploading(true);
    try {
      ...
      const { error } = await supabase.storage.from('school-documents').upload(path, file, { upsert: true });
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6" onClick={e => e.stopPropagation()}>
        <h2 className="text-lg font-bold text-gray-800 mb-1">אישור תשלום משכורת</h2>
        <p className="text-gray-500 text-sm mb-5">{employee.name} — {monthLabel} — {formatCurrency(employee.monthlySalary)}</p>
        <div className="space-y-4">
          <div>
            <label className="label">קובץ אסמכתא (אופציונלי)</label>
            <div
        ...
        <div className="flex gap-3 mt-6">
          <button onClick={handlePay} disabled={uploading} className="btn-primary flex-1 justify-center">
            <CheckCircle size={14} />
...
export default function SalariesPage() {
  const { user, school, currentYear } = useApp();
  ...
  const addEmployee = async (data) => {
    if (isMockMode) {
      setEmployees(prev => [...prev, { ...data, id: `emp${Date.now()}`, isActive: true }]);
      return;
    }
  ...
  const deleteEmployee = async (id) => {
    ...
    if (isMockMode) { setEmployees(prev => prev.filter(e => e.id !== id)); return; }
  ...
  const handlePayment = async (employee, month, data) => {
    ...
    if (isMockMode) {
      setPayments(prev => ({ ...prev, [key]: { status: data.status, receiptUrl: data.receiptUrl, notes: data.notes } }));
      return;
    }
    ...
    if (existing?.id) {
      await supabase.from('salary_payments').update(dbData).eq('id', existing.id);
    } else {
      const { data: row } = await supabase.from('salary_payments').insert(dbData).select().single();
      if (row) dbData.id = row.id;
    }
        ...
        <div className="space-y-4">
          ...
          {employees.length === 0 && (
            <div className="card text-center py-12 text-gray-400">
              <p className="mb-2">אין עובדים</p>
              <p className="text-sm">הוסיפי עובדים בלשונית "עובדים"</p>
            </div>
          )}
        </div>
      )}
                  ...
                  <tr key={emp.id} className="hover:bg-gray-50">
                    ...
                    {(user?.role === 'principal' || user?.role === 'admin') && (
                      <td className="px-3 py-3 text-left">
                        <button onClick={() => deleteEmployee(emp.id)} className="p-1.5 hover:bg-gray-100 rounded-lg text-gray-400 hover:text-red-600 transition-colors">
                          <Trash2 size={14} />
                        </button>
                      </td>
                    )}
